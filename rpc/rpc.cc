...
std::map<unsigned int, unsigned int> reply_window_deleted;
                ...
                unsigned int xid_rep, char **b, int *sz)
{
        ...
        rt=NEW;
        if(list_rt.empty()){
                reply_window_deleted[clt_nonce] = 0;
        }else{
                for(it=list_rt.begin(); it!=list_rt.end(); it++){
                        if(it->xid == xid){
                                if(it->cb_present == true){
                                        ...
                                        *sz = it->sz;
                                        rt = DONE;
                                }else if(it->cb_present == false){
                                        rt = INPROGRESS;
                                }
                                break;
                        }
                }
                if(xid < reply_window_deleted[clt_nonce]) rt=FORGOTTEN;
        }
        if(rt == NEW){
                reply_t new_rt(xid);
        ...
        for(it = list_rt.begin(); it != list_rt.end(); it++){
                if(it->xid <= xid_rep){
                        ...
                        free((*it).buf);
                        it = list_rt.erase(it);
                        if(reply_window_deleted[clt_nonce] < xid_rep){
                                reply_window_deleted[clt_nonce] = xid_rep;
                        }
                }
        }
